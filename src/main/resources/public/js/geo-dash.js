...
$(function () {
    ...
    if (projAOI) {
        ...
        catch (e) { console.warn("missing projAOI"); }
    }
...
function completeGraph() {
    pageWidgets.forEach(function (widget, index) {
        if (widget.properties[0] == "timeSeriesGraph") {
            ...
            catch (e) { console.warn("Graph failed: " + e.message); }
        }
    });
}
function addBuffer(whichMap) {
    ...
    catch (e) { console.warn("buffer failed"); }
}
var theDash;
function fillDashboard(dashboard) {
    ...
    for (i = 0; i < pageWidgets.length; i++) {
        ...
        if (pageWidgets[i].properties[0] == "addImageCollection") {
            ...
            var tl = ol.proj.transform([projAOI[0], projAOI[1]], "EPSG:4326", "EPSG:3857");
            var br = ol.proj.transform([projAOI[2], projAOI[3]], "EPSG:4326", "EPSG:3857");
            // projAOI = tl.concat(br);
            ...
            $.ajax({
                ...
                contentType: "application/json",
                data: JSON.stringify({
                    collectionNameTimeSeries: pageWidgets[i].properties[1],
                    polygon: eval(projPairAOI),
                    indexName: pageWidgets[i].properties[4],
                    visParams: visParams,
                    dateFromTimeSeries: pageWidgets[i].properties[2],
                    dateToTimeSeries: pageWidgets[i].properties[3]
                })
            }).fail(function (jqXHR, textStatus, errorThrown) {
            ...
            $.ajax({
                ...
                async: true,
                indexVal: i,
                polyVal: eval(projPairAOI),
                crossDomain: true,
                contentType: "application/json",
                data: JSON.stringify({
                    paramType: collectionName,
                    paramValue: eval(projPairAOI)
                })
            }).fail(function (jqXHR, textStatus, errorThrown) {
                console.warn(textStatus);
            }).done(function (data, _textStatus, _jqXHR) {
...
function addWidget(widget) {
    ...
    if (widget.properties[0]) {
        ...
        var widgettitle = $("<h4 />", { "id": "widgettitle_" + widget.id }).html(widget.name);
...
function imageCollectionAJAX(url, id, collectionName, visParams, dateFrom, dateTo) {
    $.ajax({
        ...
        console.warn("error from imageCollection: " + textStatus);
    }).done(function (data, _textStatus, _jqXHR) {
        if (data.errMsg) {
