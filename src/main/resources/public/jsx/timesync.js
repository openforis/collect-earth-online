...
function TrajectoryPanel({ annualData, allData, domain, showAll, vertices, setVertices, stretch }) {
    ...
    useEffect(() => {
        const svgTrajectory = d3.select(d3Container.current);
        ...
        const allCircles = svgTrajectory.selectAll(".allData")
            .data(allData)
            .enter()
            .append("circle")
        ...
        const circles = svgTrajectory.selectAll(".data")
            .data(annualData)
            .enter()
            .append("circle")
        ...
        svgTrajectory.append("path") //local because it will get overwritten
            .attr("d", lineFunction(lineData))
            .attr("id", "plotLine");

        const doyTxt = svgTrajectory.append("text")
            .attr("id", "doyTxt")
...
function ChipViewer({ chipInfo, toggleVertex, chipSettings, setChipSettings }) {
    ...
    return (
        <div
            className={`chipHolder annual ${isVertex ? "selected" : ""}`}
...
function ChipGallery () {
    ...
    const [chipSettings, setChipSettings] = useState(defaultChipSettings);
