...
CREATE OR REPLACE FUNCTION get_all_users_by_institution_id(_institution_id integer)
 RETURNS TABLE(
    ...
    INNER JOIN institution_users
        ON users.id = institution_users.user_id
    INNER JOIN roles
        ON roles.id = institution_users.role_id
...
CREATE OR REPLACE FUNCTION get_user(_email text)
 RETURNS TABLE(
    ...
    SELECT id, email, password, administrator, reset_key
    FROM users
    WHERE email = _email
...
CREATE OR REPLACE FUNCTION get_user_stats(_user_email text)
 RETURNS TABLE (
    ...
    WITH users_plots as (
        ...
        SELECT count(DISTINCT proj_id)::int as proj_count,
...
CREATE OR REPLACE FUNCTION add_institution_migration(_institution_id integer, _name text, _logo text, _description text, _url text, _archived boolean)
 RETURNS integer AS $$
    ...
    INSERT INTO institutions
        (id, name, logo, description, url, archived)
    VALUES
        (_institution_id, _name, _logo, _description, _url, _archived)
    RETURNING id
...
CREATE OR REPLACE FUNCTION add_institution_imagery(_institution_id integer, _visibility text, _title text, _attribution text, _extent jsonb, _source_config jsonb)
 RETURNS integer AS $$
    ...
    INSERT INTO imagery
        (institution_id, visibility, title, attribution, extent, source_config)
    VALUES
...
CREATE OR REPLACE FUNCTION add_institution_imagery_migration(_imagery_id integer, _institution_id integer, _visibility text, _title text, _attribution text, _extent jsonb, _source_config jsonb)
 RETURNS integer AS $$
    ...
    INSERT INTO imagery
        (id, institution_id, visibility, title, attribution, extent, source_config)
    VALUES
        (_imagery_id, _institution_id, _visibility, _title, _attribution, _extent, _source_config)
    RETURNING id
...
CREATE OR REPLACE FUNCTION add_project_widget(_project_id integer, _dashboard_id uuid, _widget jsonb)
 RETURNS integer AS $$
    ...
    INSERT INTO project_widgets
        (project_id, dashboard_id, widget)
    VALUES
...
CREATE OR REPLACE FUNCTION select_project(_id integer)
 RETURNS setOf project_return AS $$
    ...
    SELECT
        ...
        availability,              name,
        description,               privacy_level,
        ST_AsGeoJSON(boundary),    base_map_source,
        plot_distribution,         num_plots,
        plot_spacing,              plot_shape,
        plot_size,                 sample_distribution,
        samples_per_plot,          sample_resolution,
        survey_questions,          survey_rules,
        classification_times,      FALSE as editable
    FROM projects
    WHERE id = _id
...
CREATE OR REPLACE FUNCTION flag_plot(_plot_id integer, _user_id integer, _confidence integer)
 RETURNS integer AS $$
    ...
    INSERT INTO user_plots
        (user_id, plot_id, flagged, confidence, collection_time)
    VALUES
...
CREATE OR REPLACE FUNCTION select_limited_project_plots(_project_id integer, _maximum integer)
 RETURNS setOf plots_return AS $$
    ...
    SELECT all_plots.id, all_plots.project_id,
...
CREATE OR REPLACE FUNCTION select_plot_samples(_plot_id integer, _project_id integer)
 RETURNS TABLE (
    ...
    SELECT samples.id,
        ...
        sample_values.imagery_id,
        sample_values.imagery_attributes
    FROM samples
...
CREATE OR REPLACE FUNCTION add_user_samples(
    ...
    WITH user_plot_table AS (
        INSERT INTO user_plots
            (user_id, plot_id, confidence, collection_start, collection_time)
        VALUES
    ...
    INSERT INTO sample_values
        (user_plot_id, sample_id, imagery_id, imagery_attributes, value)
...
CREATE OR REPLACE FUNCTION add_sample_values_migration(_user_plot_id integer, _sample_id integer, _value jsonb, _imagery_id integer, _imagery_attributes jsonb)
 RETURNS integer AS $$
    ...
    INSERT INTO sample_values
        (user_plot_id, sample_id, value, imagery_id, imagery_attributes)
    VALUES
        ( _user_plot_id, _sample_id, _value, _imagery_id, _imagery_attributes)
    RETURNING id
...
CREATE OR REPLACE FUNCTION add_user_plots_migration(_plot_id integer, _username text, _flagged boolean, _collection_start timestamp, _collection_time timestamp)
 RETURNS integer AS $$
    ...
    INSERT INTO user_plots
        (plot_id, flagged, collection_start, collection_time, user_id)
...
CREATE OR REPLACE FUNCTION dump_project_plot_data(_project_id integer)
 RETURNS TABLE (
    ...
    WITH all_rows AS (
        SELECT pl.id as m_plot_id, samples.id as m_samples_id, pl.ext_id as pl_ext_id, *
...
CREATE OR REPLACE FUNCTION add_plots_by_json(_project_id integer, _json_data text)
 RETURNS integer AS $$
    ...
    WITH jvalue AS (
        ...
        SELECT create_project_plot(_project_id, ST_SetSRID(ST_GeomFromGeoJSON(center), 4326) ) as plot_id,
