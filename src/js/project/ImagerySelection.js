            ...
            filteredImageryList.map((imagery, idx) => (
                <div className="col-md-6 form-check" key={idx}>
                    <input
                        ...
                        checked={
                            projectImageryList.includes(imagery.id) ||
                            imagery.id === defaultImageryId
                        }
                    />
                    <label htmlFor={imagery.id} className="form-check-label">
                        {imagery.title}
                    </label>
                </div>
            ))
                    ...
                    <div id="project-imagery">
                        <div className="form-group" id="project-default-imagery">
                            ...
                            <select
                                ...
                                value={imageryId || -1}
                                onChange={(e) =>
                                    setProjectDetails({imageryId: parseInt(e.target.value)})
                                }
                            >
                                {institutionImagery.map((imagery, uid) => (
                                    <option key={uid} value={imagery.id}>
                                        {imagery.title}
                                    </option>
                                ))}
                            </select>
                        ...
                        <div className="form-group">
                            ...
                            {renderImageryRow(
                                institutionImagery.filter(
                                    (imagery) => imagery.visibility === "public"
                                ),
                                imageryId,
                                projectImageryList,
                                setProjectDetails,
                            ...
                            {renderImageryRow(
                                institutionImagery.filter(
                                    (imagery) => imagery.visibility === "private"
                                ),
                                imageryId,
                                projectImageryList,
                                setProjectDetails,
                        ...
                        <p id="project-imagery-text" className="font-italic ml-2 small">
                            * Institution imagery will only be visible to institution members, no
                            matter the project visibility.
                        </p>
                    </div>
                ) : (
