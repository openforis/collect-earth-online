...
export class SurveyQuestionDesign extends React.Component {
    ...
    getChildQuestionIds = (questionId) => {
        const childQuestions = this.context.surveyQuestions.filter(
            (sq) => sq.parentQuestion === questionId
        );
        return childQuestions.length === 0
            ? [questionId]
            : childQuestions.reduce((acc, cur) => [...acc, ...this.getChildQuestionIds(cur.id)], [
                  questionId,
              ]);
    };
    ...
    removeQuestion = (questionId) => {
        ...
        const newSurveyQuestions = this.context.surveyQuestions.filter(
            (sq) => !questionsToRemove.includes(sq.id)
        );
        this.context.setProjectDetails({surveyQuestions: newSurveyQuestions});
    };

    removeAnswer = (questionId, answerId) => {
        const matchingQuestion = this.context.surveyQuestions.find(
            (sq) => sq.parentQuestion === questionId && sq.parentAnswer === answerId
        );
        if (matchingQuestion) {
            alert(
            ...
            const newSurveyQuestions = this.context.surveyQuestions.map((sq) =>
                sq.id === updatedQuestion.id ? updatedQuestion : sq
            );
            this.context.setProjectDetails({surveyQuestions: newSurveyQuestions});
        }
    };
            ...
            <div id="survey-design">
                <SurveyCardList
                    ...
                    newAnswerComponent={(surveyQuestion) =>
                        surveyQuestion.answers.length <
                            this.maxAnswers(
                                surveyQuestion.componentType,
                                surveyQuestion.dataType
                            ) && (
                            <NewAnswerDesigner
                                setProjectDetails={this.context.setProjectDetails}
                                surveyQuestions={this.context.surveyQuestions}
...
class NewQuestionDesigner extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (this.props.surveyQuestions.length !== prevProps.surveyQuestions.length) {
            if (
                !this.props.surveyQuestions.find(
                    (question) => question.id === this.state.selectedParent
                )
            ) {
    ...
    addSurveyQuestion = () => {
        if (this.state.newQuestionText !== "") {
            ...
            const repeatedQuestions = surveyQuestions.filter(
                (sq) => removeEnumerator(sq.question) === this.state.newQuestionText
            ).length;
    ...
    render() {
        const selectedParent = this.props.surveyQuestions.find(
            (question) => question.id === this.state.selectedParent
        );
        return (
                    ...
                    <tr>
                        ...
                        <td>
                            <select
                                ...
                                size="1"
                                onChange={(e) =>
                                    this.setState({selectedParent: parseInt(e.target.value)})
                                }
                    ...
                    <tr>
                        ...
                        <td>
                            <select
                                ...
                                size="1"
                                onChange={(e) =>
                                    this.setState({selectedAnswer: parseInt(e.target.value)})
                                }
                            ...
                            >
                                ...
                                {this.state.selectedParent > 0 && selectedParent
                                    ? selectedParent.answers.map((answer) => (
...
class NewAnswerDesigner extends React.Component {
    ...
    addSurveyAnswer = () => {
        ...
        if (this.state.newAnswerText.length > 0) {
            ...
            const updatedQuestion = {...surveyQuestion, answers: updatedAnswers};
            const newSurveyQuestions = surveyQuestions.map((sq) =>
                sq.id === updatedQuestion.id ? updatedQuestion : sq
            );
            ...
            <div className="NewAnswerDesigner">
                <div className="col d-flex">
                    ...
                    <input
                        ...
                        onKeyDown={(e) => {
                            if (e.key === "e" && this.props.surveyQuestion.dataType === "number")
                                e.preventDefault();
                        }}
...
export class SurveyQuestionHelp extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (
            this.context.surveyQuestions.length > 0 &&
    ...
    getChildQuestions = (currentQuestionId) => {
        ...
        return childQuestions.length === 0
            ? [question]
            : childQuestions.reduce((acc, cur) => [...acc, ...this.getChildQuestions(cur.id)], [
                  question,
              ]);
    };
    ...
    updateQuestionStatus = () => {
        ...
        this.setState({
            ...
            selectedQuestion: {
                ...
                ...visibleAnswered[this.state.selectedQuestion.id],
            },
        });
    };
    ...
    render() {
        return (
            <div className="p-3">
                <SurveyCollection
                    surveyQuestions={this.context.surveyQuestions.map((q) => ({
                        ...q,
                        answered: [],
                        visible: [],
                        ...this.state.visibleAnswered[q.id],
                    }))}
                    surveyRules={this.context.surveyRules}
