...
export default function BulkAddAnswers({ closeDialog, surveyQuestionId, surveyQuestion }) {
  ...
  const getAnswerErrors = (answerPairs) =>
    [
      last(answerPairs).length !== 2 &&
        "You must provide pairs of color and answer.  If you feel like you have correct data, check for extra separators (comma, tab, or new line)",
      answerPairs.find(([c, _a]) => !c.match(/#[0-9|a-f]{6}/)) &&
      ...
      <div className="modal-dialog" role="document">
        <div className="modal-content text-left" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-success" onClick={addAnswers} type="button">
              Add
            </button>
          </div>
        </div>
