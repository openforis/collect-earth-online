...
export default function SurveyCard({ cardNumber, editMode, surveyQuestionId, topLevelNodeIds }) {
  const [showQuestions, setShow] = useState(true);
  const { setProjectDetails, surveyQuestions } = useContext(ProjectContext);

  const swapId = (val, checkVal, swapVal) =>
    val === checkVal ? swapVal : val === swapVal ? checkVal : val;

  const swapCardOrder = (upOrDown) => {
    const originalOrder = findObject(
      surveyQuestions,
      ([id, _sq]) => Number(id) === surveyQuestionId
    )[1].cardOrder;
    const newId = topLevelNodeIds[topLevelNodeIds.indexOf(surveyQuestionId) + upOrDown];
    const newOrder = findObject(surveyQuestions, ([id, _sq]) => Number(id) === newId)[1].cardOrder;
    setProjectDetails({
      surveyQuestions: mapVals(surveyQuestions, (val) =>
        val.cardOrder !== undefined
