...
class WidgetLayoutEditor extends React.PureComponent {
    ...
    componentDidMount() {
        this.fetchProject(this.state.projectId, true).catch((response) => {
        ...
        fetch(`/get-institution-imagery?institutionId=${institutionId}`)
            ...
            .then((data) => {
                this.setState({
                    ...
                    widgetBaseMap: data
                        .map((o) => o.id.toString())
                        .includes(this.state.widgetBaseMap)
    ...
    getImageByType = (imageType) =>
        imageType === "getStats"
            ? "/img/geodash/statssample.gif"
            : !imageType || imageType.toLowerCase().includes("image")
            ? "/img/geodash/mapsample.gif"
            : imageType.toLowerCase().includes("degradationtool")
            ? "/img/geodash/degsample.gif"
        ...
        const widgets = _.map(sWidgets, (widget, i) => {
            if (widget.layout) {
                ...
                if (widget.gridrow) {
                    ...
                    h =
                        widget.gridrow.trim().split(" ")[3] !== undefined
                            ? parseInt(widget.gridrow.trim().split(" ")[3])
                            : 1;
                }
                // create .layout
    ...
    generateDOM = () => {
        ...
        return _.map(this.state.widgets, (widget, i) => (
            ...
            >
                <h3 className="widgetEditor title">
                    {widget.name}
                    <span
                        className="remove"
                        onClick={(e) => {
    ...
    onCreateNewWidget = () => {
        ...
        const id =
            this.state.widgets.length > 0
                ? Math.max.apply(
                      Math,
                      this.state.widgets.map(function (o) {
                          return o.id;
                      })
                  ) + 1
                : 0;
        ...
        widget.baseMap = this.state.imagery.filter(
            (imagery) => String(imagery.id) === this.state.widgetBaseMap
        )[0];
        if (this.state.selectedWidgetType === "DualImageCollection") {
            ...
            if (
                ["LANDSAT5", "LANDSAT7", "LANDSAT8", "Sentinel2"].includes(
                    this.state.selectedDataType
                )
            ) {
                img1.filterType =
                    this.state.selectedDataType !== null ? this.state.selectedDataType : "";
                img1.visParams = {
            ...
            if (
                ["LANDSAT5", "LANDSAT7", "LANDSAT8", "Sentinel2"].includes(
                    this.state.selectedDataTypeDual
                )
            ) {
                img2.filterType =
                    this.state.selectedDataTypeDual !== null ? this.state.selectedDataTypeDual : "";
                img2.visParams = {
            ...
            widget.filterType = "";
            widget.visParams =
                this.state.imageParams === "" ? {} : JSON.parse(this.state.imageParams);
            ...
            const wType =
                this.state.selectedWidgetType === "TimeSeries"
    ...
    setFormStateByDates = (isDual) => {
        ...
        if (!this.state.dualLayer) {
            ...
            isFormReady =
                ed > sd && ed2 > sd2 && this.state.formReady !== true
                    ? true
                    : (ed < sd || ed2 < sd2) && this.state.formReady === true
                    ? false
                    : null;
    ...
    fetchProject = (id, setDashboardID) =>
                ...
                <div className="modal fade show" style={{display: "block"}}>
                    <div className="modal-dialog" role="document">
                        <div className="modal-content">
                                ...
                                <form>
                                    <div className="form-group">
                                        ...
                                        <select
                                            ...
                                            id="widgetTypeSelect"
                                            onChange={(e) =>
                                                this.onWidgetTypeSelectChanged(e, "i am anything")
                                            }
                                        >
                                            ...
                                            <option label="Time Series Graph" value="TimeSeries">
                                                Time Series Graph
                                            </option>
                                            ...
                                            <option label="Image Asset" value="imageAsset">
                                                Image Asset
                                            </option>
                                            <option
                                            ...
                                            <option label="Polygon Compare" value="polygonCompare">
                                                Polygon Compare
                                            </option>
                                        </select>
                                    </div>
                            ...
                            <div className="modal-footer">{this.getFormButtons()}</div>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div className="form-group">
                                        ...
                                        >
                                            ...
                                            {this.state.projectList
                                                .filter(({id, name}) =>
                                                    (id + name.toLocaleLowerCase()).includes(
                                                        this.state.projectFilter.toLocaleLowerCase()
                                                    )
                                                )
                                                .map(({id, name}, uid) => (
                                                    <option key={uid} value={id}>
                                                        {id} - {name}
                                                    </option>
                                                ))}
                                        </select>
    ...
    getBaseMapSelector = () => {
        if (
            [
                ...
                "DegradationTool",
                "polygonCompare",
            ].includes(this.state.selectedWidgetType)
        ) {
            return (
                <div className="form-group">
                    <div
                    ...
                    >
                        {this.state.imagery &&
                            this.state.imagery.map(({id, title}) => (
            ...
            return (
                <React.Fragment>
                    ...
                    >
                        ...
                        <option label="NDVI" value="NDVI">
                            NDVI
                        </option>
                        <option label="EVI" value="EVI">
                            EVI
                        </option>
                        <option label="EVI 2" value="EVI2">
                            EVI 2
                        </option>
                        <option label="NDMI" value="NDMI">
                            NDMI
                        </option>
                        <option label="NDWI" value="NDWI">
                            NDWI
                        </option>
                        <option label="LANDSAT 5" value="LANDSAT5">
                            LANDSAT 5
                        </option>
                        <option label="LANDSAT 7" value="LANDSAT7">
                            LANDSAT 7
                        </option>
                        <option label="LANDSAT 8" value="LANDSAT8">
                            LANDSAT 8
                        </option>
                        <option label="Sentinel-2" value="Sentinel2">
                            Sentinel-2
                        </option>
                        <option label="Custom widget" value="Custom">
                            Custom widget
                        </option>
                    </select>
                </React.Fragment>
                ...
                return (
                    <React.Fragment>
                        ...
                        >
                            ...
                            <option label="Image Collection Asset" value="imageCollectionAsset">
                                Image Collection Asset
                            </option>
                            <option label="Custom widget" value="Custom">
                                Custom widget
    ...
    getInstitutionImageryInfo = () => (
        <div>
            ...
            <a
                href={`/review-institution?institutionId=${this.state.institutionID}`}
                target="_blank"
    ...
    getAvailableBandsControl = () =>
        this.state.availableBands.length > 0 ? (
            <div>
    ...
    getAvailableBandsControlDual = () =>
        this.state.availableBandsDual.length > 0 ? (
            <div>
                <label>Available Bands: </label>
                <br />
    ...
    getDataForm = () => {
            ...
            return (
                <React.Fragment>
                    ...
                    <div className="form-group">
                        ...
                        >
                            <option label="NDFI" value="NDFI">
                                NDFI
                            </option>
                            <option label="SWIR1" value="SWIR1">
                                SWIR1
                            </option>
                            <option label="NIR" value="NIR">
                                NIR
                            </option>
                            ...
                            <option label="SWIR2" value="SWIR2">
                                SWIR2
                            </option>
                        </select>
                    </div>
        ...
        if (
            ...
            if (
                ...
                (this.state.selectedDataType === "imageCollectionAsset" ||
    ...
    onLayoutChange = (layout) => {
        const newWidgets = this.state.widgets.map((w, i) => {
            if (_.isEqual(w.layout, layout[i])) {
                ...
                this.serveItUp(
                    `${this.state.theURI}/update-widget?widgetId=${newWidget.id}`,
                    newWidget
                );
                return newWidget;
            }
