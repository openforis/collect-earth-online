...
*** Enabling https
We recommend that you enable https to that all features work properly and securely.  These directions are for the domain collect.earth.
...
5. Edit collect-earth-online/deploy/scripts/cert_renewal.sh to replace "collect.earth" with "yourdomain.com".
6. Edit collect-earth-online/deploy/scripts/packageKeysAndRestartApps.sh to replace "collect.earth" with "yourdomain.com".
...
an application to communicate to Google Earth Engine called GEE-Gateway.
To build and launch GEE-Gateway, download the source code from
https://github.com/openforis/gee-gateway then follow these steps:
