...
- [[https://jdk.java.net][Java Development Kit (version 17)]]
...
There can only be one superuser account with ~user_uid~ of ~1~ ~adminstrator~ flag set to ~true~.
Activating this flag for other users can cause their queries to sometimes not return the full result, as some of those queries assume only one root superuser to be existing.
